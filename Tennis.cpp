    ...
    while (window.isOpen())
    {
        ...
        while (window.pollEvent(event))
        {
            ...
            if (event.type == sf::Event::KeyPressed)
            {
                if (event.key.code == sf::Keyboard::Num1)
                {
                    difficulty = 1;
                    ballSpeed = easySpeed;
                    selectedDifficultyMessage.setString("Difficulty: Easy");
                }
                if (event.key.code == sf::Keyboard::Num2)
                {
                    difficulty = 2;
                    ballSpeed = mediumSpeed;
                    selectedDifficultyMessage.setString("Difficulty: Medium");
                }
                if (event.key.code == sf::Keyboard::Num3)
                {
                    difficulty = 3;
                    ballSpeed = hardSpeed;
                    selectedDifficultyMessage.setString("Difficulty: Hard");
                }

                // Pause/Unpause the game
                if (event.key.code == sf::Keyboard::P && isPlaying)
                {
                    isPaused = !isPaused;
                    if (isPaused)
                    {
        ...
        if (isPlaying && !isPaused)
        {
            ...
            if (leftScore >= 5 || rightScore >= 5)
            {
                isPlaying = false;
                if (leftScore >= 5)
                    pauseMessage.setString("Congratulations, You Won!\n\n" + inputString);
                else
                    pauseMessage.setString("You Lost!\n\n" + inputString);
            }
        }
        ...
        {
            // Draw the pause message and difficulty selection
            window.draw(pauseMessage);
            window.draw(sfmlLogo);
            window.draw(scoreText);  // Display the score even when paused
            window.draw(difficultyMessage);
            window.draw(selectedDifficultyMessage);
        }
